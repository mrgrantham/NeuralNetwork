...
public:
  Trainer(std::shared_ptr<Layer> model,
      ...
      : trainingInput_(trainingInput), trainingOutput_(trainingOutput),
        trainingOrder_(trainingInput_.size()), model_(model),
        learningRate_(0.1f), randomDevice_(), generator_(randomDevice_()) {
  ...
  void train(int epochs) {
    ...
    for (int epoch = 0; epoch < epochs; epoch++) {
      ...
      for (int trainingOrderIndex = 0;
           trainingOrderIndex < trainingInput_.size(); trainingOrderIndex++) {
        ...
        while (currentLayer->inputLayer_) {
          ...
          for (int nodeIndex = 0; nodeIndex < currentLayer->nodeCount();
            ...
            for (int inputNodeIndex = 0;
                 inputNodeIndex < inputLayer->nodeCount(); inputNodeIndex++) {
